...
def compute_edges(frames, img_size=IMG_SIZE):
    ...
    for frame in frames:
        ...
        # Compute Canny edges (thresholds can be tuned)
...
class VideoDataGenerator(Sequence):
    ...
    def __getitem__(self, index):
        ...
        edge_batch = np.zeros((len(batch_video_paths), self.num_frames, self.img_size, self.img_size, 1), dtype=np.float32)
        ...
        for i, path in enumerate(batch_video_paths):
            ...
            edge_batch[i] = compute_edges(frames, self.img_size)

        y = np.array(batch_labels, dtype=np.float32)
        return ((rgb_batch, flow_batch, edge_batch), y)
...
with strategy.scope():
    ...
    # Build branch for edge input
    edge_input = keras.Input(shape=(NUM_FRAMES, IMG_SIZE, IMG_SIZE, 1))
    edge_branch = build_3d_cnn_branch((NUM_FRAMES, IMG_SIZE, IMG_SIZE, 1), base_filters=16)
    edge_features = edge_branch(edge_input)

    # Fuse features from all branches and add Dense layers.
    fused = layers.Concatenate()([rgb_features, flow_features, edge_features])
    x = layers.Dense(64, activation='relu', kernel_initializer='he_normal')(fused)
    x = layers.Dropout(0.5)(x)
    x = layers.Dense(32, activation='relu', kernel_initializer='he_normal')(x)
    x = layers.Dropout(0.5)(x)
    output = layers.Dense(NUM_CLASSES, activation='sigmoid', dtype='float32')(x)  # Cast to float32

    model = keras.Model(inputs=[rgb_input, flow_input, edge_input], outputs=output)
